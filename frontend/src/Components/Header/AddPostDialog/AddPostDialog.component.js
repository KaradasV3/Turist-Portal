...
const AddPostDialog = ({ isOpen, closeModal }) => {
  ...
  const onPictureChange = (event) => setPicture(event.target.value);
  ...
  const handleAdd = async () => {
    try {
      ...
      toast.success(`Your post was successfully added. You'll see your post when admin accept it.`);
    } catch (ex) {
  ...
  const formIsValid = Boolean(category) && Boolean(country) && Boolean(city);
  ...
  return (
    <Dialog open={isOpen} onClose={closeModal} maxWidth="md">
      <DialogTitle>Adding post form</DialogTitle>
      <DialogContent>
        <Grid spacing={2} container justify="center">
          <Grid item xs={12}>
            <DialogContentText>Please complete this form to add new post.</DialogContentText>
          </Grid>
          <Grid item xs={12}>
            <Autocomplete
              freeSolo
      ...
      <DialogActions>
        ...
        <Button onClick={handleAdd} disabled={!formIsValid} color="primary" variant="contained">
          ADD
        </Button>
      </DialogActions>
    </Dialog>
